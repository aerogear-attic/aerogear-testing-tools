...
public abstract class VariantContext<
        ENTITY,
        ENTITY_ID,
        ...
        extends AbstractUPSContext<ENTITY, ENTITY_ID, BLUEPRINT, EDITOR, PARENT, WORKER, CONTEXT> {

    public VariantContext(WORKER worker, PARENT parent, Session session) {
        super(worker, parent, session);
    }

    public CONTEXT resetSecret(ENTITY_ID variantID) {
        getWorker().resetSecret(castInstance(), variantID);
        return castInstance();
    }
}
